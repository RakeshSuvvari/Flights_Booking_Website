...
def book(request):
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            flight1 = Flight.objects.get(id=flight_1)
            flight2 = Flight.objects.get(id=flight_2) if f2 else None
            ...
            for i in range(1, passengerscount + 1):
                ...
                seat1 = request.POST.get(f'passenger{i}Seat1', '')
                seat2 = request.POST.get(f'passenger{i}Seat2', '') if f2 else None

                passenger = Passenger.objects.create(
                    first_name=fname,
                    last_name=lname,
                    gender=gender.lower(),
                    seat_number_one=seat1,
                    seat_number_two=seat2
                )
            ...
            try:
                ticket1 = createticket(
                    ...
                    flight1, flight_1date, flight_1class,
                    coupon, countrycode, email, mobile
                )
                if f2:
                    ticket2 = createticket(
                        request.user, passengers, passengerscount,
                        flight2, flight_2date, flight_2class,
                        coupon, countrycode, email, mobile
                    )

                # Fare Calculation
                if flight_1class == 'Economy':
                    fare = flight1.economy_fare * passengerscount
                    if f2:
                        fare += flight2.economy_fare * passengerscount
                elif flight_1class == 'Business':
                    fare = flight1.business_fare * passengerscount
                    if f2:
                        fare += flight2.business_fare * passengerscount
                elif flight_1class == 'First':
                    fare = flight1.first_fare * passengerscount
                    if f2:
                        fare += flight2.first_fare * passengerscount

            except Exception as e:
                return HttpResponse(f"Booking failed: {e}")
